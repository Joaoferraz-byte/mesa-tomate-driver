...
All use `bmRequestType=0x21`, `bRequest=0x09`, `wValue=0x0308`, and a 250 ms timeout. The reference drivers send `wIndex=2`, but the activator now **auto-detects the control interface** at runtime: it is the first HID interface whose altsetting exposes an interrupt IN endpoint (the same endpoint the mx002 and vin1060plus drivers read back after each write). The semantic assignment of the bytes is partially inferred; what is confirmed is the sequence observed in public code and the association of the 8-byte report with a mode change.

## PC-Mode Handshake and Robustness

The tablet boots in basic mode and the vendor Windows service transitions it to work mode right after enumeration, which is why the Linux helper reproduces the same startup posture before sending the reports. The activator detaches `hid-generic` from all interfaces, performs a USB device reset (as the `mx002_linux_driver` does), re-applies configuration 1, claims **all** HID interfaces, sends the selected sequence to the auto-detected control interface, **reads the interrupt IN endpoint after every report** (the vendor firmware answers each mode report; the answer is logged), releases the interfaces, and then re-attaches the kernel driver so the kernel re-probes the control interface with the new work-mode descriptor, exposing pen proximity without a replug; if any step fails it retries up to three times with a 500 ms backoff. Reconnect the tablet if the session is left in an inconsistent state.
